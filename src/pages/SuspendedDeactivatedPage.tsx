...
export function SuspendedDeactivatedPage({ onBack }: { onBack: () => void }) {
  ...
  // Find IMEIs with suspended or deactivated status
  const statusIMEIs = useMemo(() => {
    return Array.from(imeiNotesMap.values())
      .filter(n => n.suspended || n.deactivated)
      .map(n => n.imei);
  }, [imeiNotesMap]);
  ...
  const filteredSummaries = useMemo(
    () => allSummaries.filter(s => statusIMEIs.includes(s.imei)),
    [allSummaries, statusIMEIs]
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h2 className="text-2xl font-bold mb-4">Suspended / Deactivated IMEIs</h2>
        <IMEISummaryTable
