...
export function DeactivatedIMEIPage({ onBack }: { onBack: () => void }) {
  ...
  // Find IMEIs that are deactivated
  const deactivatedIMEIs = useMemo(() => {
    return Array.from(imeiNotesMap.values())
      .filter(n => n.deactivated)
      .map(n => n.imei);
  }, [imeiNotesMap]);

  // Get all summaries and filter to deactivated IMEIs
  const allSummaries = useMemo(() => getIMEISummaries(), [getIMEISummaries]);
  const deactivatedSummaries = useMemo(
    () => allSummaries.filter(s => deactivatedIMEIs.includes(s.imei)),
    [allSummaries, deactivatedIMEIs]
  );

  // Filter by search
  const filteredSummaries = useMemo(() => {
    if (!searchTerm.trim()) return deactivatedSummaries;
    const lower = searchTerm.toLowerCase();
    return deactivatedSummaries.filter(s => 
  ...
  }, [deactivatedSummaries, searchTerm, imeiNotesMap]);
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-2xl">Deactivated IMEIs</CardTitle>
                <p className="text-sm text-muted-foreground mt-1">
                  {filteredSummaries.length} deactivated {filteredSummaries.length === 1 ? 'IMEI' : 'IMEIs'}
                </p>
              </div>
